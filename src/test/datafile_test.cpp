...
TEST(Datafile, StringData)
{
...

	{
		...
		EXPECT_EQ(Writer.AddDataString(""), -1);  
		EXPECT_EQ(Writer.AddDataString("Abc"), 0);
		EXPECT_EQ(Writer.AddDataString("DDNet最好了"), 1);
		EXPECT_EQ(Writer.AddDataString("aβい🐘"), 2);
		EXPECT_EQ(Writer.AddData(3, "Abc"), 3);  
		EXPECT_EQ(Writer.AddData(7, "foo\0bar"), 4);  
		EXPECT_EQ(Writer.AddData(5, "xyz\xff\0"), 5);  
		EXPECT_EQ(Writer.AddData(4, "XYZ\xff"), 6);  

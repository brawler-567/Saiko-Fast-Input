...
{
	EXPECT_TRUE(str_utf8_isspace(0x200b));  
...
TEST(Str, Utf8CompConfusables)
{
	...
	EXPECT_FALSE(str_utf8_comp_confusable("am", "Arn") == 0);
	EXPECT_TRUE(str_utf8_comp_confusable("l", "ӏ") == 0);  
	EXPECT_TRUE(str_utf8_comp_confusable("i", "¡") == 0);  
...
TEST(Str, Utf8ToSkeleton)
{
	...
	EXPECT_EQ(aBuf[0], 'r');
	EXPECT_EQ(aBuf[1], 'n');
	EXPECT_EQ(str_utf8_to_skeleton("ӏ", aBuf, std::size(aBuf)), 1);  
	EXPECT_EQ(aBuf[0], 'i');
	EXPECT_EQ(str_utf8_to_skeleton("¡", aBuf, std::size(aBuf)), 1);  
	EXPECT_EQ(aBuf[0], 'i');
...
TEST(Str, Utf8ToLowerCodepoint)
{
	...
	EXPECT_TRUE(str_utf8_tolower_codepoint(192) == 224);  
	EXPECT_TRUE(str_utf8_tolower_codepoint(7882) == 7883);  
}
...
TEST(Str, Utf8CompNocase)
{
	...
	EXPECT_TRUE(str_utf8_comp_nocase("ölüa", "ÖlÜ") > 0);  

...
int CSnapshot::GetExternalItemType(int InternalType) const
{
	...
	int TypeItemIndex = GetItemIndex(InternalType);  
...
const void *CSnapshot::FindItem(int Type, int Id) const
{
	...
	if(Type >= OFFSET_UUID)
	{
		...
		for(int i = 0; i < m_NumItems; i++)
		{
			...
			if(pItem->Type() == 0 && pItem->Id() >= OFFSET_UUID_TYPE)  
			{
...
int CSnapshotDelta::CreateDelta(const CSnapshot *pFrom, const CSnapshot *pTo, void *pDstData)
{
	...
	{
		const CSnapshotItem *pCurItem = pTo->GetItem(i);  
		aPastIndices[i] = GetItemIndexHashed(pCurItem->Key(), aHashlist);  
	}

	for(int i = 0; i < NumItems; i++)
	{
		 
		const int ItemSize = pTo->GetItemSize(i);  
		const CSnapshotItem *pCurItem = pTo->GetItem(i);  
...
bool CSnapshotBuilder::AddExtendedItemType(int Index)
{
	...
	int *pUuidItem = static_cast<int *>(NewItem(0, GetTypeFromIndex(Index), sizeof(CUuid)));  

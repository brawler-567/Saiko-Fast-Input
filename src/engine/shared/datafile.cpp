...
bool CDataFileReader::Open(class IStorage *pStorage, const char *pFilename, int StorageType)
{
	...
	AllocSize += (int64_t)Header.m_NumRawData * sizeof(void *);  
	AllocSize += (int64_t)Header.m_NumRawData * sizeof(int);  
...
void CDataFileWriter::Finish()
{
	...
	const int64_t OffsetSize = (m_vItems.size() + m_vDatas.size() * 2) * sizeof(int);  

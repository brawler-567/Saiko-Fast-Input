...
{
	str_format(pOut, Size, "%s %f", m_pScriptName, Value);
}
...
void SStringConfigVariable::Serialize(char *pOut, size_t Size, const char *pValue) const
{
	...
	EscapeParam(pOut + OutLen, pValue, Size - OutLen - 1);  
	str_append(pOut, "\"", Size);
}
...
#define MACRO_CONFIG_FLT(Name, ScriptName, Def, Min, Max, Flags, Desc) \
	{ \
		const char *pHelp = Min == Max ? Desc " (default: " #Def ")" : (Max == 0.0f ? Desc " (default: " #Def ", min: " #Min ")" : Desc " (default: " #Def ", min: " #Min ", max: " #Max ")"); \

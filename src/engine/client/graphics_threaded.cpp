...
{
	dbg_assert(pFilename[0] != '\0', "Cannot load texture from file with empty filename");  
...
{
	return (unsigned char)(std::clamp(ColorComponent, 0.0f, 1.0f) * 255.0f + 0.5f);  
}
...
void CGraphics_Threaded::DrawRectExt(float x, float y, float w, float h, float r, int Corners)
{
	...
	CQuadItem aQuads[9];
	NumItems = 0;
	aQuads[NumItems++] = CQuadItem(x + r, y + r, w - r * 2, h - r * 2);  
	aQuads[NumItems++] = CQuadItem(x + r, y, w - r * 2, r);  
	aQuads[NumItems++] = CQuadItem(x + r, y + h - r, w - r * 2, r);  
	aQuads[NumItems++] = CQuadItem(x, y + r, r, h - r * 2);  
	aQuads[NumItems++] = CQuadItem(x + w - r, y + r, r, h - r * 2);  

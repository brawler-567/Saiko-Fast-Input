...
static inline bool RepackMsg(const CMsgPacker *pMsg, CPacker &Packer, bool Sixup)
{
	...
	{
		Packer.AddInt(pMsg->m_System ? 1 : 0);  
...
void CServer::DoSnapshot()
{
	...
	for(int i = 0; i < MaxClients(); i++)
	{
...

		{
			...
			char aData[CSnapshot::MAX_SIZE];
			CSnapshot *pData = (CSnapshot *)aData;  
...
void CServer::SendCapabilities(int ClientId)
{
	...
	Msg.AddInt(SERVERCAPFLAG_DDNET | SERVERCAPFLAG_CHATTIMEOUTCODE | SERVERCAPFLAG_ANYPLAYERFLAG | SERVERCAPFLAG_PINGEX | SERVERCAPFLAG_ALLOWDUMMY | SERVERCAPFLAG_SYNCWEAPONINPUT);  
	SendMsg(&Msg, MSGFLAG_VITAL, ClientId);
}
...
#define ADD_INT(p, x) \
	...
	ADD_INT(p, minimum(MaxClientsProtocol, maximum(MaxClients - maximum(Config()->m_SvSpectatorSlots, Config()->m_SvReservedSlots), PlayerCount)));  
	ADD_INT(p, ClientCount);  
	ADD_INT(p, minimum(MaxClientsProtocol, maximum(MaxClients - Config()->m_SvReservedSlots, ClientCount)));  
...
#define RESET() \
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_aClients[i].IncludedInServerInfo())
		{
			...
			q.AddString(ClientName(i), MAX_NAME_LENGTH);  
			q.AddString(ClientClan(i), MAX_CLAN_LENGTH);  

			ADD_INT(q, m_aClients[i].m_Country);  
			...
			ADD_INT(q, Score);  
			ADD_INT(q, GameServer()->IsClientPlayer(i) ? 1 : 0);  
			if(Type == SERVERINFO_EXTENDED)
				q.AddString("", 0);  
...
void CServer::CacheServerInfoSixup(CCache *pCache, bool SendClients, int MaxConsideredClients)
{
	...
	if(Config()->m_Password[0])  
		Flags |= SERVER_FLAG_PASSWORD;
	Packer.AddInt(Flags);

	int MaxClients = m_NetServer.MaxClients();
	Packer.AddInt(Config()->m_SvSkillLevel);  
	Packer.AddInt(PlayerCount);  
	Packer.AddInt(maximum(MaxClients - maximum(Config()->m_SvSpectatorSlots, Config()->m_SvReservedSlots), PlayerCount));  
	Packer.AddInt(ClientCount);  
	Packer.AddInt(maximum(MaxClients - Config()->m_SvReservedSlots, ClientCount));  
	...
	if(SendClients)
	{
		for(int i = 0; i < MaxConsideredClients; i++)
		{
			if(m_aClients[i].IncludedInServerInfo())
			{
				Packer.AddString(ClientName(i), MAX_NAME_LENGTH);  
				Packer.AddString(ClientClan(i), MAX_CLAN_LENGTH);  
				Packer.AddInt(m_aClients[i].m_Country);  
				Packer.AddInt(m_aClients[i].m_Score.value_or(-1));  
				Packer.AddInt(GameServer()->IsClientPlayer(i) ? 0 : 1);  
...
int CServer::Run()
{
	 ...
	 
	{
		...
		while(m_RunServer < STOPPING)
		{
			...
			if(m_MapReload || m_SameMapReload || m_CurrentGameTick >= MAX_TICK)  
			{
...
int CServer::MaplistEntryCallback(const char *pFilename, int IsDir, int DirType, void *pUser)
{
	...
	aFilename[FilenameLength] = '\0';  
	if(FilenameLength >= sizeof(CMaplistEntry().m_aName))  
		return 0;

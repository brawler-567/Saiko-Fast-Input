...
int main(int argc, const char *argv[])
{
	...
	str_copy(aaMapNames[0], argv[1]);  
	str_copy(aaMapNames[1], argv[4]);  
	str_copy(aaMapNames[2], argv[9]);  

	float aaaGameAreas[2][2][2];

	for(int i = 0; i < 2; i++)
	{
		aaaGameAreas[i][0][0] = str_tofloat(argv[2 + i * 3]) * 32;  
		aaaGameAreas[i][1][0] = str_tofloat(argv[3 + i * 3]) * 32;  
		aaaGameAreas[i][0][1] = aaaGameAreas[i][0][0] + str_tofloat(argv[7]) * 32;  
		aaaGameAreas[i][1][1] = aaaGameAreas[i][1][0] + str_tofloat(argv[8]) * 32;  
	}

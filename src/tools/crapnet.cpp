...
static int g_ConfigInterval = 10;  
...
static void Run(unsigned short Port, NETADDR Dest)
{
	...
	while(true)
	{
		...
		while(true)
		{
			...
			{
				p->m_aData[6 + (rand() % (Bytes - 6))] = rand() & 255;  
				if((rand() % 10) == 0)
				{
		...
		while(true)
		{
			...
			if((time_get() - p->m_Timestamp) > g_CurrentLatency)
			{
				...
				g_CurrentLatency = ((time_freq() * MsPing) / 1000) + (int64_t)(((time_freq() * MsFlux) / 1000) * Flux);  

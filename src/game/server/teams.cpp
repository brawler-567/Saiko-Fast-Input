	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_Core.Team(i) == Team && GameServer()->m_apPlayers[i])
		{
			...
			if(i != ExceptId)
			{
				...
				{
					GameServer()->m_apPlayers[i]->Respawn(true);  
				}
			}
		}
	}
...
void CGameTeams::OnFinish(CPlayer *pPlayer, int TimeTicks, const char *pTimestamp)
{
	...
	else if(pData->m_BestTime.has_value())  
	{
...
void CGameTeams::OnCharacterSpawn(int ClientId)
{
	...
	if(!IsValidTeamNumber(Team) || !m_aTeamLocked[Team])
	{
		if(g_Config.m_SvTeam != SV_TEAM_FORCED_SOLO)
			...
			SetForceCharacterTeam(ClientId, ClientId);  
...
{
	return Team >= TEAM_FLOCK && Team < NUM_DDRACE_TEAMS - 1;  
}

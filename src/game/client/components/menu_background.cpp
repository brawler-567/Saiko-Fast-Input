...
int CMenuBackground::ThemeScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(str_comp(aThemeName, "none") == 0 || str_comp(aThemeName, "auto") == 0 || str_comp(aThemeName, "rand") == 0)  
		return 0;
...
std::vector<CTheme> &CMenuBackground::GetThemes()
{
	if(m_vThemes.empty())  
	{
		...
		m_vThemes.emplace_back("", true, true);  
		LoadThemeIcon(m_vThemes.back());

		m_vThemes.emplace_back("auto", true, true);  
		LoadThemeIcon(m_vThemes.back());

		m_vThemes.emplace_back("rand", true, true);  
		LoadThemeIcon(m_vThemes.back());

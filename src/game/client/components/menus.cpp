...
int CMenus::DoButton_CheckBoxAutoVMarginAndSet(const void *pId, const char *pText, float *pValue, CUIRect *pRect, float VMargin)
{
	CUIRect CheckBoxRect;
	pRect->HSplitTop(VMargin, &CheckBoxRect, pRect);

	int Logic = DoButton_CheckBox_Common(pId, pText, *pValue > 0.0f ? "X" : "", &CheckBoxRect, BUTTONFLAG_LEFT);

	if(Logic)
		*pValue = *pValue > 0.0f ? 0.0f : 1.0f;
...
void CMenus::RenderPopupFullscreen(CUIRect Screen)
{
	...
	else if(m_Popup == POPUP_RENAME_DEMO)
	{
		...
		if(DoButton_Menu(&s_ButtonOk, Localize("Ok"), 0, &Ok) || Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER))
		{
			...
			else if(str_utf8_comp_nocase(aBufOld, aBufNew) != 0 &&  

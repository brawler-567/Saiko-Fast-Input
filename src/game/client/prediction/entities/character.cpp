...
void CCharacter::FireWeapon()
{
	...
	case WEAPON_HAMMER:
	{
		...
		for(int i = 0; i < Num; ++i)
		{
			...
			{
				if(m_GameTeam == pTarget->m_GameTeam && pTarget->m_LastSnapWeapon == WEAPON_NINJA)  
				{
					Force.x *= 50 * 0.01f;
					Force.y *= 50 * 0.01f;
				}
		...
		if(GameWorld()->m_WorldConfig.m_IsVanilla)
		{
			...
			for(int i = -ShotSpread; i <= ShotSpread; ++i)
			{
				...
				new CProjectile(
					...
					ProjStartPos,  
					direction(a) * Speed,  
					(int)(GameWorld()->GameTickSpeed() * GlobalTuning()->m_ShotgunLifetime),  
					false,  
					false,  
					-1  
				);
...
void CCharacter::Read(CNetObj_Character *pChar, CNetObj_DDNetCharacter *pExtended, bool IsLocal)
{
	...
	if(distance(m_PrevPos, m_Pos) > 10.f * 32.f)  
		m_PrevPos = m_Pos;

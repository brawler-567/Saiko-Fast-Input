...
void CGameClient::OnInit()
{
	...
	for(int i = 0; i < g_pData->m_NumImages; i++)
	{
		...
		else if(g_pData->m_aImages[i].m_pFilename[0] == '\0')  
...
void CGameClient::OnNewSnapshot()
{
	...
	{
		if(m_Camera.m_ZoomSmoothingTarget > m_Camera.m_Zoom)  
			ShowDistanceZoom = m_Camera.m_ZoomSmoothingTarget;
		else if(m_Camera.m_ZoomSmoothingTarget < m_Camera.m_Zoom && m_LastShowDistanceZoom > 0)  
...
void CGameClient::OnPredict()
{
	...
	int FinalTickSelf = FinalTickRegular + (int)std::ceil(g_Config.m_TcFastInput);  
	int FinalTickOthers = FinalTickRegular;  
	if(g_Config.m_TcFastInput > 0.0f && g_Config.m_TcFastInputOthers)
	...
	for(int Tick = Client()->GameTick(g_Config.m_ClDummy) + 1; Tick <= FinalTickSelf; Tick++)
	{
		...
		if(g_Config.m_TcFastInput > 0.0f && Tick > FinalTickRegular)
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			if(CCharacter *pChar = m_PredictedWorld.GetCharacterById(i))
			{
				if(Tick > FinalTickOthers && i != m_Snap.m_LocalClientId && (!pDummyChar || i != pDummyChar->GetCid()))
					pChar->Destroy();
				else
				{
		...
		absolute(Client()->GameTick(g_Config.m_ClDummy) - Client()->PrevGameTick(g_Config.m_ClDummy)) <= 2)
	{
		...
		float SmoothPace = 4 - 1.5f * PredTime / 800.f;  
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(in_range(length(PredErr), 0.05f, 5.f))
			{
				...
				for(int j = 0; j < 2; j++)
				{
					int64_t Remaining = minimum((1.f - aMixAmount[j]) * Len, minimum(time_freq() * 0.700f, (1.f - aMixAmount[j ^ 1]) * Len + time_freq() * 0.300f));  
	...
	{
		int PredTime = std::clamp(Client()->GetPredictionTime(), 0, 8000);  
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			else
			{
				...
				Confidence = std::pow(Confidence, 4.0f);  
			}
			...
			float TickSize = TickDuration / ((float)PredTime * PredTimeScale);  
			...
			NewConfidence = std::clamp(NewConfidence, MinConfidence, 1.0f);  
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(Predict() && (i == m_Snap.m_LocalClientId || (AntiPingPlayers() && !IsOtherTeam(i))) && pChar)
		{
			...
			if(i == m_Snap.m_LocalClientId || (PredictDummy() && i == m_aLocalIds[!g_Config.m_ClDummy]))
			{
				...
				if(g_Config.m_TcFastInput > 0.0f)
					Pos = GetSmoothPos(i);
...
vec2 CGameClient::GetSmoothPos(int ClientId)
{
	...
	if(g_Config.m_TcFastInput > 0.0f && (ClientId == m_Snap.m_LocalClientId || g_Config.m_TcFastInputOthers))
	{
		float TotalSmoothTick = (SmoothTick - 1) + SmoothIntra + g_Config.m_TcFastInput;
...
vec2 CGameClient::GetFreezePos(int ClientId)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if(SmoothTick > 0 &&
			...
			m_aClients[ClientId].m_aPredTick[SmoothTick % 200] <= Client()->PredGameTick(g_Config.m_ClDummy) + (int)std::ceil(g_Config.m_TcFastInput))
		{
			Pos[i] = mix(m_aClients[ClientId].m_aPredPos[(SmoothTick - 1) % 200][i], m_aClients[ClientId].m_aPredPos[SmoothTick % 200][i], SmoothIntra);
		}
	}

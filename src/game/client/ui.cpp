...
const CLinearFloatScrollbarScale CUi::ms_LinearFloatScrollbarScale;
...
bool CUi::DoScrollbarOption(const void *pId, float *pOption, const CUIRect *pRect, const char *pStr, float Min, float Max, const IFloatScrollbarScale *pScale, unsigned Flags, const char *pSuffix)
{
	const bool Infinite = Flags & CUi::SCROLLBAR_OPTION_INFINITE;
	const bool NoClampValue = Flags & CUi::SCROLLBAR_OPTION_NOCLAMPVALUE;
	const bool MultiLine = Flags & CUi::SCROLLBAR_OPTION_MULTILINE;
	...
	float Value = (DelayUpdate && m_pLastActiveScrollbar == pId && CheckActiveItem(pId)) ? m_ScrollbarValue : *pOption;
	if(Infinite)
	{
	...
	if(DelayUpdate && m_pLastActiveScrollbar == pId && CheckActiveItem(pId))
	{
		...
		m_ScrollbarValue = (int)Value; 
		return false;
	}

	if(absolute(*pOption - Value) > 0.0001f)
	{
		*pOption = Value;
		return true;
	}

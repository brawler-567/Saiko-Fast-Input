...
void CEditor::DoQuad(int LayerIndex, const std::shared_ptr<CLayerQuads> &pLayer, CQuad *pQuad, int Index)
{
	...
	static std::vector<SAlignmentInfo> s_PivotAlignments;  
	static std::vector<SAlignmentInfo> s_vAABBAlignments;  
	static SAxisAlignedBoundingBox s_SelectionAABB;  
...
void CEditor::DoQuadPoint(int LayerIndex, const std::shared_ptr<CLayerQuads> &pLayer, CQuad *pQuad, int QuadIndex, int V)
{
	...
	static std::vector<SAlignmentInfo> s_Alignments;  
	static ivec2 s_LastOffset;
...
void CEditor::RenderEnvelopeEditorColorBar(CUIRect ColorBar, const std::shared_ptr<CEnvelope> &pEnvelope)
{
	...
	Graphics()->QuadsEnd();
	Ui()->ClipDisable();
	ColorBarBackground.h -= Ui()->Screen()->h / Graphics()->ScreenHeight();  
...
void CEditor::RenderMenubar(CUIRect MenuBar)
{
	...
	if(Map()->m_Modified)
	{
		...
		DoButtonLogic(&s_ChangedIndicator, 0, &ChangedIndicator, BUTTONFLAG_NONE, "This map has unsaved changes.");  
	}
...
bool CEditor::Append(const char *pFilename, int StorageType, bool IgnoreHistory)
{
	...
	const auto &&Rename = [&](const std::shared_ptr<CEditorImage> &pImage) {
		...
		while(std::find_if(Map()->m_vpImages.begin(), Map()->m_vpImages.end(), [aRenamed](const std::shared_ptr<CEditorImage> &OtherImage) { return str_comp(OtherImage->m_aName, aRenamed) == 0; }) != Map()->m_vpImages.end())
			str_format(aRenamed, sizeof(aRenamed), "%s (%d)", pImage->m_aName, DuplicateCount++);  

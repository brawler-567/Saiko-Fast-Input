...
bool io_read_all(IOHANDLE io, void **result, unsigned *result_len)
{
	...
	int64_t len = real_len < 0 ? 1024 : real_len;  
	char *buffer = (char *)malloc(len + 1);
	int64_t read = io_read(io, buffer, len + 1);  
	if(read < len)
	{
...
int fs_storage_path(const char *appname, char *path, int max)
{
	...
	if(SHGetKnownFolderPath(FOLDERID_RoamingAppData, 0,   nullptr, &wide_home) != S_OK)
	{
